...
WITH LineData AS (
    ...
    SELECT
        ...
        CASE WHEN 'HISTORY' = 'HISTORY' THEN ([Quantity] - [Outstanding Quantity]) * [Unit Cost]
    ...
    WHERE
        ...
        AND NOT ('HISTORY' = 'HISTORY' AND [Quantity] - [Outstanding Quantity] = 0)
    ...
    SELECT
        ...
        ([Quantity] - [Outstanding Quantity]) * CASE WHEN [Qty_ per Unit of Measure] = 0 THEN 1 ELSE [Qty_ per Unit of Measure] END AS [Quantity Delivered],
        'US020' AS [Subsidiary]
    FROM [dbo].[IPG Medical Corporation$Purchase History Line]
    WHERE
        [Order Date] > '2019-01-01'
        AND [Quantity] > 0
        AND [Unit Cost (LCY)] > 0
    ...
    SELECT
        ...
        ([Quantity] - [Outstanding Quantity]) * CASE WHEN [Qty_ per Unit of Measure] = 0 THEN 1 ELSE [Qty_ per Unit of Measure] END AS [Quantity Delivered],
        'US020' AS [Subsidiary]
    FROM [dbo].[IPG Medical Corporation$Purchase Line]
    WHERE
        [Order Date] > '2019-01-01'
        AND [Quantity] > 0
        AND [Unit Cost (LCY)] > 0
    ...
    SELECT
        ...
        ([Quantity] - [Outstanding Quantity]) * CASE WHEN [Qty_ per Unit of Measure] = 0 THEN 1 ELSE [Qty_ per Unit of Measure] END AS [Quantity Delivered],
        'CA010' AS [Subsidiary]
    FROM [dbo].[IPG Canada$Purchase History Line]
    WHERE
        [Order Date] > '2019-01-01'
        AND [Quantity] > 0
        AND [Unit Cost (LCY)] > 0
    ...
    SELECT
        ...
        ([Quantity] - [Outstanding Quantity]) * CASE WHEN [Qty_ per Unit of Measure] = 0 THEN 1 ELSE [Qty_ per Unit of Measure] END AS [Quantity Delivered],
        'CA010' AS [Subsidiary]
    FROM [dbo].[IPG Canada$Purchase Line]
    WHERE
        [Order Date] > '2019-01-01'
        AND [Quantity] > 0
        AND [Unit Cost (LCY)] > 0
...
ReceiptTable AS (
    ----------------------------------------
    -- Combine US010, US020, CA010 Purch_ Rcpt_ Line
    ----------------------------------------
    SELECT 
        [Line No_],
        [Order No_] AS [Order #],
